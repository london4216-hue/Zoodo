...
// Zoodo gets his OWN distinct character voice — the bright, upbeat built-in
// 'sunny' voice — so he never sounds like the warm female lesson narrator
// (ElevenLabs Rachel / built-in honey). This is the one place a different,
// more animated voice is correct. Using the built-in 'sunny' voice keeps
// Zoodo distinct from the narrator regardless of the ElevenLabs key state.
...
const GOOFY_PERSONA = `You are Zoodo — the cheerful, bubbly learning buddy of EduPath AI, a wiggly purple creature greeting a toddler.

PERSONALITY: Fun, light, joyful, and full of sunshine. Bouncy and playful with a few giggles ("hee hee", "yay"), but never over-the-top silly. Warm and encouraging — like a favorite playful friend.
...
// Generates a short, goofy, giggly greeting for the kid, spoken in Zoodo's
// own bright, upbeat character voice (distinct from the lesson narrator).
export default async function(req): Promise<Response> {
  try {
    ...
    const prompt = GOOFY_PERSONA + '\n\n' +
      ...
      `It MUST start with exactly: "Hi ${kidName}... let's get ready to ${action}!" ` +
