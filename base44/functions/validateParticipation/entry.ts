...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    const prompt =
      ...
      `Return JSON: { "success": boolean, "observed": a very short description of what you see, "feedback": a warm 1-2 sentence message spoken directly to ${kidName} — celebrating with their name if success, or gently encouraging another try if not. No stage directions, just the words to say. }`;
